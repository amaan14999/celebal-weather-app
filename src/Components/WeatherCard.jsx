...
const WeatherCard = ({
  ...
  useEffect(() => {
    if (iconString) {
      if (iconString.toLowerCase().includes("cloud")) {
        ...
        setIcon(wind);
      } else if (iconString.toLowerCase().includes("shower")) {
        setIcon(shower);
      }
    }
  }, [iconString]);
  ...
  return (
    <div className="w-[22rem] min-w-[22rem] min-h-max glassCard p-4">
      ...
      <div className="font-semibold w-full flex justify-between items-center mt-4">
        <p className="flex-1 text-center p-2">{new Date().toDateString()}</p>
        <p className="flex-1 text-center p-2">{time}</p>
      </div>
      <div className="w-full flex justify-between items-center mt-4 gap-4">
        <p className="flex-1 text-center p-2 font-bold bg-teal-600 shadow rounded-lg">
          Wind Speed <p className="font-normal">{windspeed} km/h</p>
        </p>
        <p className="flex-1 text-center p-2 font-bold rounded-lg bg-pink-400">
